...
check_group_snap_exists()
{
    ...
    $XMLSTARLET sel -Q -t -v "//group_snaps/group_snap[snapshot='${snap}']" < "$CMD_STDOUT" || { fail 'Snap "'"${snap}"'" not found' ; return 1; }
}
...
wait_for_group_replay_stopped()
{
    ...
    wait_for_group_replay_state "${cluster}" "${group_spec}" 'stopped' "${image_count}"
}
...
get_newest_group_mirror_snapshot_id() 
{
    ...
    _group_snap_id=$(xmlstarlet sel -t -v "//group_snaps/group_snap[state='complete' and position()=last()]/snapshot" "$CMD_STDOUT" | awk -F. '{print $NF}')
}
...
wait_for_group_replay_complete()
{
    ...
    if [ "${MIRROR_IMAGE_MODE}" = "snapshot" ]; then
        wait_for_group_snapshot_sync_complete ${local_cluster} ${cluster} ${group_spec}
    else
        return 1
    fi
}
...
test_group_and_image_sync_status()
{
    ...
    while true; do
        for s in 0.4 1 1 1 2 2 2 4 8 16 16 32 32; do
            ...
            get_newest_group_mirror_snapshot_id "${local_cluster}" "${group_spec}" local_group_snap_id
            local local_group_snap_state=$(xmlstarlet sel -t -v "//group_snap/state" < ${local_status_log})
...
test_group_status_in_pool_dir()
{
    ...
    run_admin_cmd "rbd --cluster ${cluster} mirror group status ${group_spec} --format xml --pretty-format"
    ...
    if [ -n "${image_count}" ]; then 
        actual_image_count=$($XMLSTARLET sel -t -v "count(//images/image/status)" "$CMD_STDOUT")
        started_image_count=$($XMLSTARLET sel -t -v "count(//images/image/status[contains(text(), 'replaying')])" "$CMD_STDOUT")
