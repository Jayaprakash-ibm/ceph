...
test_empty_groups()
{
  ...
  # next step - seems to end up in 'up+stopped' TODO DEFECT
  wait_for_group_status_in_pool_dir "${secondary_cluster}" "${pool}"/"${group0}" 'up+replaying' 0
...
test_images_different_pools()
{
  ...
  if [ -z "${RBD_MIRROR_USE_RBD_MIRROR}" ]; then
    wait_for_group_status_in_pool_dir "${primary_cluster}" "${pool0}"/"${group}" 'down+unknown' 2
  fi
...
test_create_group_with_large_image()
{
  ...
  if [ -z "${RBD_MIRROR_USE_RBD_MIRROR}" ]; then
    wait_for_group_status_in_pool_dir "${primary_cluster}" "${pool0}"/"${group}" 'down+unknown' 1
  fi
...
# this test PASSES (with a sleep in it)
...
# this test PASSES, but does show up a defect - see comment in test
