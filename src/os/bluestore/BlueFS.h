...
public:
  ...
  void add_usage(void* hint, const bluefs_extent_t& extent) override
  {
    ...
    Mode m = mode.load(std::memory_order_relaxed);
    if (m == Mode::NORMAL) {
      RocksDBBlueFSVolumeSelector::add_usage(hint, extent);
      return;
    }
    bluefs_extent_t forced = extent;
    forced.bdev = m == Mode::FORCE_SLOW ? BlueFS::BDEV_SLOW
      : (m == Mode::FORCE_DB ? BlueFS::BDEV_DB : extent.bdev);
    RocksDBBlueFSVolumeSelector::add_usage(hint, forced);
  }
  ...
  void sub_usage(void* hint, const bluefs_extent_t& extent) override
  {
    ...
    Mode m = mode.load(std::memory_order_relaxed);
    if (m == Mode::NORMAL) {
      RocksDBBlueFSVolumeSelector::sub_usage(hint, extent);
      return;
    }
    bluefs_extent_t forced = extent;
    forced.bdev = m == Mode::FORCE_SLOW ? BlueFS::BDEV_SLOW
      : (m == Mode::FORCE_DB ? BlueFS::BDEV_DB : extent.bdev);
    RocksDBBlueFSVolumeSelector::sub_usage(hint, forced);
  }
